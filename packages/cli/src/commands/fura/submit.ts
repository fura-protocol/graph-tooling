...
export default class SubmitCommand extends Command {
  static description = 'Submit a subgraph to fura graph node.';
  ...
  async run() {
    ...
    } = await this.parse(SubmitCommand);
